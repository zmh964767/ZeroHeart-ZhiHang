...
function addNewPage(doc, currentPageNum, resumeName, M) {
...
function checkAndAddPage(doc, currentY, requiredHeight, currentPageNum, resumeName, M) {
...
function measureTextHeight(doc, text, fontSize, width, fontName = "NotoSC") {
  if (!text) return 0;
...
async function main() {
  ...
  if (!input.resume || !input.outputPath) {
    console.error("Missing required fields: resume, outputPath");
    process.exit(1);
  }

  const { resume, outputPath } = input;
