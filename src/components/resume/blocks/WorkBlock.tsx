...
import { WorkBlock as WorkBlockType } from "@/lib/resume/types";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Button } from "@/components/ui/button";
import { Plus, Trash2, ChevronDown, ChevronRight } from "lucide-react";
import { useResumeStore } from "@/stores/resume";
import { defaultWork } from "@/lib/resume/types";
...
export function WorkBlockEditor({ data }: WorkBlockProps) {
  const { updateContent } = useResumeStore();
  const [localData, setLocalData] = useState<WorkBlockType[]>(data);
  ...
  const handleChange = (index: number, field: keyof WorkBlockType, value: string) => {
  ...
  const handleDateChange = (index: number, field: "startDate" | "endDate", value: string) => {
    ...
    updateContent({ work: newData });
  };

  const handleBlur = (index: number, field: keyof WorkBlockType, value: string) => {
  ...
  const handleRemove = (index: number) => {
    ...
    setLocalData(newData);
    updateContent({ work: data.filter((_, i) => i !== index) });
  };

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-gray-900">工作经历</h3>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={handleAdd}
      ...
      {localData.length === 0 && (
        <div className="text-center py-8 rounded-3xl" style={{ 
          ...
          <p className="text-sm text-gray-500">暂无工作经历，点击添加</p>
        </div>
      )}
