...
import { InternshipBlock as InternshipBlockType } from "@/lib/resume/types";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Button } from "@/components/ui/button";
import { Plus, Trash2, ChevronDown, ChevronRight } from "lucide-react";
import { useResumeStore } from "@/stores/resume";
import { defaultInternship } from "@/lib/resume/types";

interface InternshipBlockProps {
  data: InternshipBlockType[];
}
...
export function InternshipBlockEditor({ data }: InternshipBlockProps) {
  const { updateContent } = useResumeStore();
  const [localData, setLocalData] = useState<InternshipBlockType[]>(data);
  ...
  const handleChange = (index: number, field: keyof InternshipBlockType, value: string) => {
  ...
  const handleDateChange = (index: number, field: "startDate" | "endDate", value: string) => {
    ...
    updateContent({ internship: newData });
  };

  const handleBlur = (index: number, field: keyof InternshipBlockType, value: string) => {
  ...
  const handleAdd = () => {
    const newItem = { ...defaultInternship };
    const newIndex = localData.length;
    setLocalData([...localData, newItem]);
    updateContent({ internship: [...data, newItem] });
  ...
  const handleRemove = (index: number) => {
    ...
    setLocalData(newData);
    updateContent({ internship: data.filter((_, i) => i !== index) });
  };
    ...
    <div className="space-y-4">
      ...
      {localData.length === 0 && (
        <div className="text-center py-8 rounded-3xl" style={{ 
          ...
          <p className="text-sm text-gray-500">暂无实习经历，点击添加</p>
        </div>
      )}
          ...
          {expandedItems.has(index) && (
            <div className="p-4 md:p-5 space-y-4 border-t" style={{ borderColor: "rgba(255, 154, 158, 0.15)" }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                ...
                <div>
                  <label className="text-sm text-gray-600 mb-2 block">职位</label>
                  <Input
                    value={item.position}
              ...
              <div>
                <label className="text-sm text-gray-600 mb-2 block">工作描述</label>
                <Textarea
                  value={item.description}
                  onChange={(e) => handleChange(index, "description", e.target.value)}
                  onBlur={(e) => handleBlur(index, "description", e.target.value)}
                  placeholder="描述你的主要工作内容和成就..."
                  rows={3}
                  style={{ 
                    background: "rgba(255, 255, 255, 0.6)",
                    backdropFilter: "blur(8px)",
